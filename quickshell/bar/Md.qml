...
Singleton {
    ...
    // Статичний білий текст (без адаптації — за рішенням користувача).
    // Через парсер-QML (onSurface: "x" трактується як signal handler) — через функції.
    function _textMain() { return "#e8eae6" }
